...
void readEncoder() {
    uint16_t turnData = 0;
    uint8_t pressData = 0;
    uint8_t prevPressData = 0;
    RotaryEncoder_init();

    while (1) {
        RotaryEncoder_getDiff(&turnData);
        RotaryEncoder_getStatus(&pressData);

        if (turnData != 0) {
            if (turnData == 1) { // Scrolling right
                ESP_LOGI("Rotary", "Scrolled right.");
                scrollLine(0, 2);
            } else if (turnData == 255 || turnData == 65535) { // Scrolling left
                ESP_LOGI("Rotary", "Scrolled left.");
                scrollLine(1, 2);
            }
        }

        if (pressData == 7 && pressData != prevPressData) {
            if (pressData == 7) {
                ESP_LOGI("Rotary", "Button Pressed.");
                getSelectedOption();
            }
        }
        prevPressData = pressData;
